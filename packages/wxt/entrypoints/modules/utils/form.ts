...
export function getFormData<T>(systemId: 'pek' | 'sek'): T {
  ...
  formData.forEach((value, name) => {
    if (data[name as keyof Partial<T>]) {
      // 如果已存在该字段，添加逗号并附加新值
      data[name as keyof Partial<T>] = (data[name as keyof Partial<T>] +
        `,${value}`) as T[keyof T]
    } else {
      // 如果是新字段，直接赋值
  ...
  if (
    !(data['unno' as keyof Partial<T>] as string).startsWith('UN') &&
    (data['unno' as keyof Partial<T>] as string).trim().length > 0
  ) {
    ;(data['unno' as keyof Partial<T>] as string) =
      'UN' + data['unno' as keyof Partial<T>]
  }
  if (systemId === 'pek') {
    ...
    PekTypeTransMap.forEach((key) => {
      if (data[key as keyof Partial<T>] !== undefined) {
        data[key as keyof Partial<T>] = Number(
          data[key as keyof Partial<T>]
        ) as T[keyof T]
      }
    })
  } else {
