...
(async () => {
  ...
  chrome.runtime.onMessage.addListener(function (request, _sender, sendResponse) {
      ...
      (async () => {
        try {
          ...
          if (request.enablePPOCR === true) {
            ...
            console.log("PPOCR is disabled in settings.");
          }
    ...
    if (request.action == "yolo-inference") {
      (async () => {
        try {
          const result = await predict_yolo26(enablePPOCR, session, new Uint8Array(request.input), ort.Tensor, request.options);
