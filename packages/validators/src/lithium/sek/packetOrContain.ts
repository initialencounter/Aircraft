...
export function packetOrContain(
  ...
  if (otherDescribeCAddition.includes('符合包装说明967第II部分')) {
    result.push({ ok: false, result: '海陆运，描述中不应存在”锂离子电池符合包装说明967第II部分”', selector })
  }
  ...
  if (
    ...
    result.push({ ok: false, result: '与设备包装在一起，其他描述错误', selector })
  if (
    otherDescribe === '542' &&
    !otherDescribeCAddition.includes('设备内置') &&
    !isChargeBoxOrRelated
  )
    result.push({ ok: false, result: '安装在设备上，其他描述错误', selector })
  return result
}
