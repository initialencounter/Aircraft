...
export function checkManufacturer(
  ...
  if (!systemIdManufacturer) {
    return [{ ok: false, result: '获取系统制造商失败, 无法验证概要制造商', selector: '' }]
  }
  systemIdManufacturer = systemIdManufacturer.replace(/\s*\(/g, '（').replace(/\)\s*/g, '）').replace(/\s*（/g, '（').replace(/）\s*/g, '）')
  summaryManufacturer = summaryManufacturer.replace(/\s*\(/g, '（').replace(/\)\s*/g, '）').replace(/\s*（/g, '（').replace(/）\s*/g, '）')
