  ...
  describe('验证', () => {
    it('验证空运数据', () => {
      for (let i = 0; i < 86; i++) {
        ...
        if ([10, 42].includes(i) && result[0].result === '运输专用名称错误，应为Sodium ion batteries') continue;
        if ([52, 68, 85].includes(i) && result[0].result === '976 应勾选: 荷电状态≤30%' && result[1].result === '运输专用名称错误，应为Sodium ion batteries') continue;
        console.log(data)
        console.log(result)
        console.log(i)
  ...
  describe('验证', () => {
    it('验证海运数据', () => {
      for (let i = 0; i < 28; i++) {
        ...
        // if ([1,5,11].includes(i) && result[0].result.includes('描述中不应该出现单块')) continue;
        if ([11, 12, 13, 14, 22, 23, 24, 26].includes(i) && result[0].result === '结论错误，运输专用名称错误，应为Sodium ion batteries') continue;
        if ([17].includes(i) && result[0].result === '物品为电池时，描述中不应该出现单块电芯' && result[1].result === '结论错误，运输专用名称错误，应为Sodium ion batteries') continue;
        if ([25, 27].includes(i) && result[0].result === '物品为电芯时，描述中不应该出现单块电池' && result[1].result === '结论错误，运输专用名称错误，应为Sodium ion batteries') continue;
        console.log(data)
        console.log(result)
        console.log(i)
        expect(result.length).toBe(0)
  ...
  describe('验证', () => {
    it('验证海运数据', () => {
      for (let i = 0; i < 18; i++) {
        ...
        const result = checkSekSodiumBtyType(data)
        if (result.length === 0) continue;
        if ([17].includes(i) && result[0].result === '物品为电芯时，描述中不应该出现单块电池' && result[1].result === '结论错误，运输专用名称错误，应为Sodium ion batteries') continue;
        if ([9, 16].includes(i) && result[0].result === '鉴别项目8，9 错误，未勾选不适用' && result[1].result === '结论错误，运输专用名称错误，应为Sodium ion batteries') continue;
        if ([10].includes(i) && result[0].result === '电池净重误差大于5%' && result[1].result === '结论错误，运输专用名称错误，应为Sodium ion batteries') continue;
        console.log(data)
        console.log(result)
        console.log(i)
        expect(result.length).toBe(0)
