  ...
  describe('验证', () => {
    it('验证海运数据', () => {
      for (let i = 0; i < 28; i++) {
        ...
        if (result.length === 0) continue;
        // if (result.length === 1 && result[0].result.includes('技术备注为空')) continue;
        if ([1,6].includes(i)  && result[0].result === '类型为电池时，描述中不应该出现电芯') continue;
        ...
        if ([17].includes(i) && result[0].result === '类型为电池时，描述中不应该出现电芯' && result[1].result === '结论错误，运输专有名称错误，应为Sodium ion batteries') continue;
        if ([25, 27].includes(i) && result[0].result === '类型为电芯时，描述中不应该出现电池' && result[1].result === '结论错误，运输专有名称错误，应为Sodium ion batteries') continue;
        console.log(data)
        console.log(result)
        console.log(i)
        expect(result.length).toBe(0)
  ...
  describe('验证', () => {
    it('验证陆运数据', () => {
      for (let i = 0; i < 18; i++) {
        ...
        const result = checkSekSodiumBtyType(data)
        if (result.length === 0) continue;
        if ([17].includes(i) && result[0].result === '类型为电芯时，描述中不应该出现电池' && result[1].result === '结论错误，运输专有名称错误，应为Sodium ion batteries') continue;
        if ([9, 16].includes(i) && result[0].result === '鉴别项目8，9 错误，未勾选不适用' && result[1].result === '结论错误，运输专有名称错误，应为Sodium ion batteries') continue;
        if ([10].includes(i) && result[0].result === '结论错误，运输专有名称错误，应为Sodium ion batteries') continue;
        console.log(data)
        console.log(result)
        console.log(i)
        expect(result.length).toBe(0)
