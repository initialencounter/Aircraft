    ...
    thread::spawn(move || {
        ...
        for path in paths {
            ...
            if path.is_file() && path_str.ends_with(".pdf") {
                if let Some(pdf_read_result) = handle_parse_file(&app_clone.clone(), path_str) {
                    res.push(pdf_read_result);
                }
            } else {
                res.extend(handle_parse_directory(&app_clone.clone(), path_str));
            }
        }
                ...
                if let Ok(entry) = entry {
                    ...
                    if file_path.ends_with(".pdf") {
                        if let Some(pdf_read_result) =
                            handle_parse_file(&app_clone, file_path.clone())
                        {
                            res.push(pdf_read_result);
                        }
                    }
