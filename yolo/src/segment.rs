...
pub fn detect_objects_on_image(buf: Vec<u8>) -> Vec<(f32, f32, f32, f32, String, f32, Vec<Vec<u8>>)> {
...
fn process_output(
    ...
    img_width: u32,
    img_height: u32,
) -> Vec<(f32, f32, f32, f32, String, f32, Vec<Vec<u8>>)> {
    ...
    // 提取mask系数 (接下来的32列)
    let masks_output2: Array2<f32> = output0.slice(s![.., (4 + num_classes)..(4 + num_classes + 32), 0])
        .to_owned();
    
    // 处理mask原型: output1的形状是 [1, 32, 160, 160]
    let masks_output: Array2<f32> = output1.slice(s![.., .., .., 0])
        .to_owned()
        .to_shape((160 * 160, 32))
        .unwrap()
        .to_owned()
    ...
    let masks = masks_output2.dot(&masks_output)
    ...
    while boxes.len() > 0 {
        ...
        let processed_mask = process_mask(current_box.6.clone(), (current_box.0, current_box.1, current_box.2, current_box.3), img_width, img_height);
        result.push((current_box.0, current_box.1, current_box.2, current_box.3, current_box.4.to_string(), current_box.5, processed_mask));
        boxes = boxes
            .iter()
