...
impl FileManager {
    ...
    fn upload(&self, file_path: &str) -> Result<String, Box<dyn Error>> {
        ...
        if response.status().is_success() {
            Ok(response.json::<FileUploadResult>()?.id)
        } else {
            Err(format!("上传失败！{}", response.text()?))?
        }
    }
    fn delete(&self, file_id: &str) -> Result<PdfDeleteResult, Box<dyn Error>> {
        // 发送请求
        let response = self
            .client
        ...
        // 处理响应
        if response.status().is_success() {
            Ok(response.json::<PdfDeleteResult>()?)
        } else {
            Err(format!("删除失败！{}", response.text()?))?
        }
    }
    fn content(&self, file_id: &str) -> Result<String, Box<dyn Error>> {
        // 发送请求
        let response = self
            .client
        ...
        if response.status().is_success() {
            ...
            Err(format!("获取文件内容失败！{}", response.text()?))?
        }
    }
    pub fn get_file_content(&self, file_path: &str) -> Result<String, Box<dyn Error>> {
        let file_id = self.upload(file_path)?;
        let result = self.content(&file_id)?;
        self.delete(&file_id)?;
        Ok(result)
    }

    pub fn chat_with_ai(&self, file_list: Vec<String>) -> Result<String, Box<dyn Error>> {
...
export interface PekData {
    ...
    market: string
        ...
        // 处理响应
        if response.status().is_success() {
            let res: ChatResponse = response.json::<ChatResponse>()?;
...
mod tests {
    ...
    fn test_upload_pdf_to_openai() {
        ...
        let json = manage.chat_with_ai(file_list).unwrap();
        println!("json: {:?}", json);
    }
}
